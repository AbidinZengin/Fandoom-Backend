...
- **API sözleşmesi FE spec'inden geldiği için projenin geri kalanından iki fark**: (1) `itemType` JSON/query'de küçük harf (`movie`/`series`; girişte büyük/küçük fark etmez), DB'de enum adı; (2) `GET /api/trivia` liste `{"data":[...]}` zarfıyla döner (`TriviaListResponse`), diğer listeler düz `List<T>`. **Etiketler**: `tags` (≤5) hazır değer (`TriviaTag`: `BEHIND_THE_SCENES, EASTER_EGG, CASTING, GOOF, LORE` — FE bunlara ikon çizer) YA DA serbest metin (≤30, `<>` yasak) olabilir; DB'de düz String (`trivia_tags` `@ElementCollection` + `@OrderColumn`, enum kısıtı yok). `TriviaTag.normalize` hazır değere denk gelenleri kanonik ada çevirir ("behind the scenes" → `BEHIND_THE_SCENES`), gerisini yazıldığı gibi bırakır; büyük/küçük harf duyarsız tekilleştirilir. İstekte spec'teki tek `tag` de kabul edilir (`tags` ile birleşir, en az biri zorunlu); yanıtta `tag` = ilk etiket, `tags` = tümü. Migration: depo kökündeki `trivia_tags_migration.sql` (eski `trivia.tag` NOT NULL kolonunu düşürür).
