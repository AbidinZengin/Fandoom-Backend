...
**Durum:** Faz 1 tamamlandı (`a101db9` + açık eksikler `a678705`/`b9a96a7`'de kapandı; `avatarUrl` zenginleştirmesi bu turda `UserProfile`'ın `community/`'ye taşınmasıyla kapandı). Faz 2 (Community Profile, daraltılmış kapsamla) tamamlandı (`22dd686`). Faz 3 (Tag Sistemi) tamamlandı. Faz 4 henüz kod yazılmadı.
...
- ✅ **Yazar zenginleştirmesi** — `AuthorSummary(id, username, avatarUrl)`, `UserService.getUsernamesByIds` + `UserProfileService.getAvatarUrlsByUserIds` ile toplu çözülüyor. `avatarUrl` eklendi (bkz. Faz 2 notundaki güncelleme — `UserProfile` ailesi bu amaçla `account/`'tan `community/`'ye taşındı, döngüsel bağımlılık oluşmasın diye).
...
**Karar (ilk halinde netleşti): `account/UserProfile` community/'ye TAŞINMADI, olduğu yerde kaldı.** Orijinal planın ayrı bir `CommunityProfileResponse`/yeni entity inşa etme fikri terk edildi — `account/`'ın zaten var olan `UserProfile`/`ProfileStats`/`UserProfileService` altyapısı community/ verisine **service-interface üzerinden bağlandı**, hiç yeni entity yazılmadı.

**Güncelleme (2026-09-12, avatarUrl zenginleştirmesiyle birlikte): `UserProfile` ailesi (`entity`/`repository`/`service`/`mapper`/`dto`: `UserProfileResponse`, `UpdateUserProfileRequest`, `ProfileStats`) sonradan `community/`'ye TAŞINDI.** Gerekçe: `account/UserProfileServiceImpl` zaten `community/ThreadService`+`CommentService`'i inject ediyordu (`account/ → community/`); `AuthorSummary`'ye `avatarUrl` eklemek ters yönde (`community/ → account/UserProfileService`) bir bağımlılık daha gerektirdiği için ikisi birden döngüsel bağımlılık oluşturacaktı. Çözüm: `UserProfile` ailesini community/'nin kendi içine taşımak — `account/AccountController`/`PublicAccountController` artık tek yönlü olarak `community.service.UserProfileService`'e delege eder. DB tablosu (`user_profile`) değişmedi. `ProfileStats` hesaplaması hâlâ `account/`'a (like/activity sayıları için `UserLikeService.countByUserId`/`ActivityLogService.countByUserIdAndType`, interface üzerinden) bağımlı — bu artık community/'den account/'a TEK yönlü bir bağımlılık, döngü yok.
...
| `avatarUrl` yazar zenginleştirmesi (Faz 1'in açık kalan parçası) | ✅ `AuthorSummary`'ye eklendi, `UserProfile` ailesi `community/`'ye taşınarak döngüsel bağımlılık olmadan çözüldü (bkz. yukarıdaki güncelleme notu) |
...
3. ~~**`avatarUrl` yazar zenginleştirmesi**~~ ✅ tamamlandı (`UserProfile` ailesi `community/`'ye taşındı, döngüsel bağımlılık önlendi).
