...
class GroupAssignmentServiceImplTest {
    ...
    void setUp() {
        service = new GroupAssignmentServiceImpl(groupAssignmentRepository, groupRepository,
                groupAssignmentMapper, characterService);

        lenient().when(groupRepository.findById(1L))
                .thenReturn(Optional.of(Group.builder().id(1L).type(GroupType.FACTION).build()));
    ...
    void delete_found_deletesById() {
        when(groupAssignmentRepository.existsById(5L)).thenReturn(true);

        service.delete(5L);

        verify(groupAssignmentRepository).deleteById(5L);
    }

    @Test
    void listForTarget_delegatesToRepositoryAndMapsResult() {
