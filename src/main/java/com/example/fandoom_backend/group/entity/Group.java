...
// Yapıma özel genel taksonomi kaydı (Faction, Species, ...). tag/'daki Tag
// ile aynı "tip'li katalog" fikri, ama Tag global iken Group subjectType/
// subjectId ile tek bir yapıma bağlıdır (Character'daki desenle aynı).
@Entity
@Table(name = "content_group", uniqueConstraints = {
        @UniqueConstraint(name = "uk_group_slug", columnNames = "slug")
}, indexes = {
        @Index(name = "idx_group_subject", columnList = "subject_type, subject_id")
})
@Getter
@Setter
@NoArgsConstructor
